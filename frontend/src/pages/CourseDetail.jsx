  ...
  if (error || !course)
    return (
      <div className="container-x py-20 text-center">
        ...
        <Link to="/courses" className="btn-burgundy">
          Browse All Courses
        </Link>
      </div>
    );
      ...
      <section className="relative bg-burgundy-700 text-white py-20 lg:py-28 overflow-hidden">
        ...
        <div className="container-x relative z-10 grid lg:grid-cols-3 gap-10 items-center">
          <div className="lg:col-span-2">
            ...
            <p className="text-gold-400 uppercase tracking-[0.3em] text-xs font-semibold mb-2">
            ...
            <div className="flex flex-wrap gap-3 mt-6">
              <div className="bg-white/10 border border-white/20 rounded-sm px-4 py-2 text-sm flex items-center gap-2">
                <Clock className="w-4 h-4 text-gold-400" /> {course.duration}
              </div>
              <div className="bg-white/10 border border-white/20 rounded-sm px-4 py-2 text-sm flex items-center gap-2">
                <GraduationCap className="w-4 h-4 text-gold-400" /> {course.eligibility.split(';')[0]}
              </div>
              <div className="bg-gold-500 text-burgundy-900 rounded-sm px-4 py-2 text-sm font-bold flex items-center gap-2">
                <Sparkles className="w-4 h-4" /> Scholarships Available
              </div>
            </div>
          </div>
          <div className="bg-white/10 backdrop-blur border border-white/20 rounded-sm p-5">
            ...
            <p className="text-white/70 text-sm mb-4">
              Free counselling • Scholarship eligibility check • 30-minute callback
            </p>
            <LeadForm
              variant="dark"
              source={`course_${slug}`}
        ...
        <div className="container-x grid lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-12">
            ...
            {course.curriculum?.length > 0 && (
              <div>
                <p className="text-gold-600 uppercase tracking-widest text-xs font-semibold mb-2 flex items-center gap-2">
            ...
            {course.careers?.length > 0 && (
              <div>
                <p className="text-gold-600 uppercase tracking-widest text-xs font-semibold mb-2 flex items-center gap-2">
                  <Briefcase className="w-3.5 h-3.5" /> Career Outcomes
                </p>
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  {course.careers.map((c, i) => (
                    <div
                      key={i}
                      className="flex items-start gap-3 p-4 bg-burgundy-50 border border-burgundy-100 rounded-sm"
                    >
            ...
            {course.faqs?.length > 0 && (
              <div>
                <p className="text-gold-600 uppercase tracking-widest text-xs font-semibold mb-2">
                  Ask About This Course
                </p>
                <h2 className="font-display text-3xl font-bold text-burgundy-700 mb-5">
                    ...
                    return (
                      <div key={i} className="bg-cream border border-burgundy-100 rounded-sm overflow-hidden">
                        ...
                        >
                          ...
                          <ChevronRight
                            className={`w-5 h-5 text-burgundy-500 flex-shrink-0 transition-transform ${open ? 'rotate-90' : ''}`}
                          />
                        </button>
          ...
          <aside className="lg:sticky lg:top-24 self-start">
            <div className="bg-burgundy-700 text-white rounded-sm p-6">
              ...
              <p className="text-white/70 text-sm mb-5">
                Speak to our counsellor for personalized course guidance, scholarship eligibility and admission
                deadlines.
              </p>
              <Link to={`/apply?course=${course.short_title}`} className="btn-gold w-full mb-3">
                Apply for {course.short_title}
              </Link>
              <Link
                to="/scholarships"
                className="block w-full text-center border border-white/30 text-white py-2.5 rounded-sm hover:bg-white hover:text-burgundy-700 transition-all text-sm font-medium"
              >
                Check Scholarships
              </Link>
            </div>
